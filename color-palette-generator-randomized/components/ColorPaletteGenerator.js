...
function generatePalette() {
  return Array.from({ length: 6 }, () => `rgb(${Math.floor(Math.random()*256)}, ${Math.floor(Math.random()*256)}, ${Math.floor(Math.random()*256)})`)
}
        ...
        {colors.map((c, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              <div title={hex}>
      <div>{hex.slice(0, 7).toUpperCase()}</div>
      <div style={{ fontSize: "0.7rem", fontWeight: "normal" }}>{c}</div>
    </div>
